...
export function Checkout() {
  ...
  const { data: saved } = useQuery({
    queryKey: ["addresses"],
    queryFn: () => api.get<{ items: Address[] }>("/v1/users/me/addresses"),
  });
  ...
  const placeOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const order = await api.post<Order>("/v1/orders", { shipping_address: addr });

      // 2. Initiate payment.
      const pay = await api.post<CreatePaymentResult>("/v1/payments", {
        order_id: order.id,
        method,
      });
      ...
      if (method === "razorpay") {
        const sim = await api.post<{ razorpay_payment_id: string; razorpay_signature: string }>(
          `/v1/payments/${pay.payment.id}/simulate`,
        );
